...
import { getVehicleMakes, getVehicleModels, createVehicle, updateVehicle } from '../services/vehicles';
...
export function VehicleForm({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let result;
      if (mode === 'create') {
        result = await createVehicle(userId, formData);
      } else {
        if (!vehicle) {
          setError('Vehicle data is required for editing');
          return;
        }
        ...
        result = await updateVehicle(updateData);
      }
      ...
      if (result.success && result.vehicle) {
        ...
        setError(result.error?.message || `Failed to ${action} vehicle`);
      }
    } catch (err: any) {
  ...
  const submitButtonText = mode === 'create' ? 'Add Vehicle' : 'Save Changes';
